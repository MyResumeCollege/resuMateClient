...
export const Experience = () => {
    ...
    const updateEditedPeriod = (experiencePart: Partial<ExperiencePeriod>) => {
        if (editedExperiencePeriod) {
            setEditedExperiencePeriod({ ...editedExperiencePeriod, ...experiencePart })
        }
    }

    const closeAddNewPeriod = () => { setEditedExperiencePeriod(undefined); setError(undefined) };
                ...
                <div ref={ref} className="flex flex-col items-stretch py-2 px-4 bg-[#dbdbdb] rounded-md gap-[10px]">
                    <div className="flex gap-[10px]">
                        <TextInput
                            ...
                            onChange={(newJobTitle) => updateEditedPeriod(({ jobTitle: newJobTitle }))}
                        />
                        <TextInput
                            autoFocus
                            label="Employer"
                            ...
                            onChange={(newEmployer) => updateEditedPeriod(({ employer: newEmployer }))}
                        />
                    </div>
                    <div className="flex gap-[10px]">
                        <TextInput
                            ...
                            wrapperClassName="flex-1"
                            inputClassName="bg-[#c8c8c8] outline-none focus:online-none"
                            value={editedExperiencePeriod.startDate.year}
                        ...
                        <TextInput
                            ...
                            wrapperClassName="flex-1"
                            inputClassName="bg-[#c8c8c8] outline-none focus:online-none"
                            value={editedExperiencePeriod.endDate.year}
    ...
    const periodRenderer = (expPeriod: ExperiencePeriod) => {
        return editedExperiencePeriod?.id === expPeriod.id ?
            renderEditPeriod() :
            <div key={expPeriod.id} className="group flex items-center py-2 px-4 bg-[#dbdbdb] hover:bg-[#cacaca] transition-all rounded-md">
                <span className="font-bold text-md mr-[7px]">{expPeriod.jobTitle}</span>
                <span className="text-sm opacity-60">| {expPeriod.employer} </span>
                <span className="text-sm opacity-60 ml-[5px]">({expPeriod.startDate.month} {expPeriod.startDate.year} - {expPeriod.startDate.month} {expPeriod.endDate.year !== "" ? expPeriod.endDate.year : "Current"})</span>
                <div className="flex items-center gap-1 ml-auto opacity-0  group-hover:opacity-100 transition-all">
                    <svg onClick={() => setEditedExperiencePeriod(expPeriod)} className="size-5 cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" >
                        <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                    </svg>
                    <svg onClick={() => removePeriod(expPeriod)} className="size-5 cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" >
